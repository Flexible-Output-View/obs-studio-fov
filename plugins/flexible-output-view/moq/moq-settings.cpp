...
uint64_t Amount(obs_data_t *settings, const char *key)
{
	...
	blog(LOG_ERROR, "Advanced integer setting has no Field: %s", key);
	return 0;
}
...
void LogFailure(const char *what, int code)
{
	...
	blog(LOG_ERROR, "Advanced setting %s rejected (%d): %s", what, code, reason ? reason : "(no reason)");
}
...
void Read(int (*get)(uint32_t, uint64_t *), uint32_t client, long long *out, bool *ok, const char *what)
{
	...
	if (ret < 0) {
		const char *reason = moq_error();
		blog(LOG_ERROR, "Failed to read the %s default (%d): %s", what, ret, reason ? reason : "(no reason)");
		*ok = false;
		return;
	}
	*out = (long long)value;
...
const DefaultValues &LibraryDefaults()
{
	static const DefaultValues defaults = [] {
		...
		if (client < 0) {
			blog(LOG_ERROR, "Failed to create a libmoq client to read defaults from: %d", client);
			return d;
		}
		...
		if (ret < 0) {
			blog(LOG_ERROR, "Failed to read the WebSocket fallback default: %d", ret);
			ok = false;
		}
...
int CreateClient(obs_data_t *settings)
{
	...
	if (!LibraryDefaults().loaded) {
		blog(LOG_ERROR, "Advanced settings unavailable: libmoq did not report its defaults");
		return -1;
	}
