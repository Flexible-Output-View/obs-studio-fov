...
bool MoQOutput::Start()
{
	...
	if (!service) {
		blog(LOG_ERROR, "%s", "Failed to get service from output");
		state->SignalStop(OBS_OUTPUT_ERROR);
		return false;
	}

	if (!obs_output_can_begin_data_capture(output, 0)) {
		blog(LOG_ERROR, "%s", "Cannot begin data capture");
		return false;
	}

	if (!obs_output_initialize_encoders(output, 0)) {
		blog(LOG_ERROR, "%s", "Failed to initialize encoders");
		return false;
	}
	...
	if (url.empty()) {
		blog(LOG_ERROR, "%s", "Server URL is empty");
	...
	if (!found_encoder) {
		blog(LOG_ERROR, "%s", "Failed to get video encoder");
		return false;
	}
	...
	blog(LOG_INFO, "Connecting to MoQ server: %s", MoQRedactUrl(url).c_str());
	...
	if (handle < 0) {
		...
		blog(LOG_ERROR, "Failed to initialize MoQ server: %d: %s", handle, reason ? reason : "unknown error");
	...
	if (superseded) {
		...
		blog(LOG_ERROR, "MoQ session failed before the output started: %s", MoQRedactUrl(url).c_str());
		return false;
	}

	blog(LOG_INFO, "Publishing broadcast: %s", path.c_str());
	...
	if (broadcast < 0) {
		blog(LOG_ERROR, "Failed to publish broadcast to session: %d", broadcast);
		broadcast = 0;
...
void MoQOutput::SessionState::Connected(const SessionRef &ref, int connect_epoch)
{
	...
	blog(LOG_INFO, "MoQ session connected (%d ms, epoch %d): %s", ms, connect_epoch, MoQRedactUrl(ref.url).c_str());
}
...
void MoQOutput::SessionState::Closed(const SessionRef &ref, int code)
{
	...
	if (code == 0) {
		blog(LOG_INFO, "MoQ session closed: %s", MoQRedactUrl(ref.url).c_str());
	} else {
		blog(LOG_ERROR, "MoQ session failed (%d): %s: %s", code, MoQRedactUrl(ref.url).c_str(), reason.c_str());
	}
...
void MoQOutput::AudioData(struct encoder_packet *packet)
{
	...
	if (pts < 0) {
		blog(LOG_WARNING, "Dropping audio frame with negative PTS: %lld", (long long)packet->pts);
		return;
	}
	...
	if (result < 0) {
		blog(LOG_ERROR, "Failed to write audio frame: %d", result);
		return;
	}
...
void MoQOutput::VideoData(struct encoder_packet *packet)
{
	...
	int64_t pts = packet->pts + packet->timebase_den / packet->timebase_num;
	if (pts < 0) {
		blog(LOG_WARNING, "Dropping video frame with negative PTS: %lld", (long long)packet->pts);
		return;
	}
	...
	auto result = moq_publish_media_frame(handle, packet->data, packet->size, pts_us);
	if (result < 0) {
		blog(LOG_ERROR, "Failed to write video frame: %d", result);
		return;
	}
...
void MoQOutput::VideoInit(obs_encoder_t *encoder)
{
	...
	if (!obs_encoder_get_extra_data(encoder, &extra_data, &extra_size)) {
		blog(LOG_WARNING, "%s", "Failed to get extra data");
	}
	...
	if (handle < 0) {
		blog(LOG_ERROR, "Failed to initialize video track: %d", handle);
		return;
	}

	blog(LOG_INFO, "Video track initialized (%ux%u, %d kbps)", video_width, video_height, video_bitrate_kbps);
}

void MoQOutput::AudioInit(obs_encoder_t *encoder)
{
	if (!encoder) {
		blog(LOG_ERROR, "%s", "Failed to get audio encoder");
		return;
	}
	...
	OBSDataAutoRelease settings = obs_encoder_get_settings(encoder);
	if (!settings) {
		blog(LOG_ERROR, "%s", "Failed to get audio encoder settings");
		return;
	}
	...
	*/
	...
	if (handle < 0) {
		blog(LOG_ERROR, "Failed to initialize audio track: %d", handle);
		return;
	}

	blog(LOG_INFO, "%s", "Audio track initialized successfully");
}

void register_moq_output()
{
	const uint32_t base_flags = OBS_OUTPUT_ENCODED | OBS_OUTPUT_SERVICE | OBS_OUTPUT_MULTI_TRACK_VIDEO | OBS_OUTPUT_MULTI_TRACK_AV | OBS_OUTPUT_MULTI_TRACK_AUDIO;
